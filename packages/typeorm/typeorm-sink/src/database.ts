...
import type {DataSink} from '@sqd-sdk/core/pipeline'
...
type A = ReadableStream


export class TypeormDatabase<T> implements DataSink<T> {
    readonly con: ReadonlyDataSource
    ...
    constructor(options: TypeormDatabaseOptions) {
        ...
        this.statusSchema = options.statusSchema || 'squid_processor'
        this.supportsHotBlocks = options.supportsHotBlocks !== false
    ...
    private async initTransaction(): Promise<void> {
        ...
        await this.con.transaction(this.isolationLevel, async (em) => {
            await em.query(`CREATE SCHEMA IF NOT EXISTS ${schema}`)
            await em.query(
                `CREATE TABLE IF NOT EXISTS ${schema}.status (id int4 primary key, height int4 not null, hash text DEFAULT '0x', nonce int4 DEFAULT 0)`,
            )
            await em.query(
