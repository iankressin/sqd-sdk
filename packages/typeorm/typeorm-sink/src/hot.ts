...
import type {Entity, EntityClass} from './store'
...
export async function rollbackBlock(statusSchema: string, em: EntityManager, blockHeight: number): Promise<void> {
    ...
    for (let rec of changes) {
        ...
        table = em.connection.driver.escape(table)
        switch (rec.change.kind) {
            ...
            case 'update': {
                let setPairs = Object.keys(rec.change.fields).map((column, idx) => {
                    return `${em.connection.driver.escape(column)} = $${idx + 1}`
                })
                if (setPairs.length) {
            ...
            case 'delete': {
                let columns = ['id', ...Object.keys(rec.change.fields)].map((col) => em.connection.driver.escape(col))
