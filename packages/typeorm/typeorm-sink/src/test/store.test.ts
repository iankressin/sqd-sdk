...
describe("Store", () => {
    describe(".save()", () => {
        ...
        it("saving a large amount of entities", async () => {
            let store = await createStore()
            let items: Item[] = []
    ...
    describe(".remove()", () => {
        ...
        it("removal by passing an entity", async () => {
            let store = await createStore()
            await store.remove(new Item('1'))
            await expect(getItemIds()).resolves.toEqual(['2', '3'])
        })

        it("removal by passing an array of entities", async () => {
            let store = await createStore()
            await store.remove([
                new Item('1'),
                new Item('3')
        ...
        it("removal by passing an id", async () => {
            let store = await createStore()
            await store.remove(Item, '1')
            await expect(getItemIds()).resolves.toEqual(['2', '3'])
        })

        it("removal by passing an array of ids", async () => {
            let store = await createStore()
    ...
    describe("Update with un-fetched reference", () => {
        useDatabase([
            'CREATE TABLE item (id text primary key , name text)',
        ...
        it(".save() doesn't clear reference (single row update)", async () => {
            let store = await createStore()
        ...
        it(".save() doesn't clear reference (multi row update)", async () => {
            let store = await createStore()
